...
struct transform_fn {
        ...
        -> std::ranges::in_out_result<std::ranges::borrowed_iterator_t<R>, std::ranges::borrowed_iterator_t<O>>
    {
        ...
        // Zip and subdivide 2 ranges in sync, this enables concurrent writing in a general case. In case of 
        ...
        // Note: To play it safe we prohibit ranges without random access, we could make zip range
        //       subdividable implicitly using `cfr::ranges::adapt` and have parallel transform work
        //       for all cases, but doing such implicit perf degradation seems risky, better let users
        //       construct the zip + for_each combo themselves if they want to do this kind of iteration.
