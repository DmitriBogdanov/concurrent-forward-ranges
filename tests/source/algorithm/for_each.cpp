...
TEST_CASE( "cfr::ranges::for_each / compile-time evaluation" ) {
    
    constexpr auto evaluate_checked = [] {
        auto range = std::array{ 0, 1, 2, 3 };
        
        cfr::ranges::for_each( range, [&]( auto && value ) { value += 10; });
...
void verify( A adaptor ) {
    ...
    const auto [iter, func] = cfr::ranges::for_each( adaptor( checked_range ), callable{} );
...
TEST_CASE( "cfr::ranges::for_each / fuzzing" ) {
